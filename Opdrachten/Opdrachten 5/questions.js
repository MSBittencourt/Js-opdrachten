...
// length is going to return the number of caracters
    return text.length;
}
...
// split make an array separated by the space, index[0] is replaced be C, and join together in a new array
    let replace = text.split(" ");
    replace[0] = 'C';
    return replace.join(' ');
}
...
var InverseString = function (text) {
    // split to make an array of each letter, reverse them and join them back in a new array.    
...
var CalcIBM = function (weight, height){
    ...
    return parseFloat(IBM.toFixed(2));
}
